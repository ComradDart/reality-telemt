...
# Свежие Ubuntu держат dpkg-lock под unattended-upgrades → apt падает сразу.
# Заставляем ВСЕ apt-операции ждать освобождения блокировки до 5 минут.
echo 'DPKG::Lock::Timeout "300";' > /etc/apt/apt.conf.d/99lock-timeout
log "Жду возможной фоновой apt-операции (unattended-upgrades)..."
apt-get update
